...
public class ChatClient_Tc extends JFrame implements ActionListener, Runnable {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnconn || e.getSource() == txtname) { // 대화명 입력 후 접속
			...
			try {
				...
				soc = new Socket("192.168.0.8", 7778);
				in = new BufferedReader(
						new InputStreamReader(
							soc.getInputStream(), "euc-kr"));
		...
		}else if(e.getSource() == txtsend || e.getSource() == btnok) {  //채팅 메시지 전송
			try {
		...
		while(true) {
			try {
				...
				if(msg.charAt(0) == '/') {
					...
					}else if(msg.charAt(1) == 'r') { //대화명 변경  //   /rkbs-jtbc
